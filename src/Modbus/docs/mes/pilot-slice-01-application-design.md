...
- `Mes.ExperienceApi` now exposes the documented operator-execution routes as a thin shared HTTP host over the existing endpoint adapter, proving that the same command and query contracts can be surfaced without leaking business logic back into the host layer.
- This reference adapter stack is intentionally not the final persistence design. It proves boundary shape, replay behavior, and host composition first, while durable SQL-backed storage and richer transport behavior remain the next implementation steps.
