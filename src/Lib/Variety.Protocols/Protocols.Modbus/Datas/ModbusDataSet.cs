...
namespace Protocols.Modbus.Datas
{
    ...
    {
        private readonly DummyDataBlock<TData, TRawData> dummyDataBlock = new();
        private readonly List<ModbusDataBlock<TData, TRawData>> dataBlocks = new();
        ...
        /// <returns>데이터 값</returns>
        /// <exception cref="ModbusException">찾을수 없을 경우</exception>
        ...
        private static void MergeNext(IList<ModbusDataBlock<TData,TRawData>> dataBlocks, ModbusDataBlock<TData, TRawData> dataBlock, int index)
        {
            ...
            var endAddresss = dataBlock.EndAddress;
            while(index < dataBlocks.Count)
            {
                var next = dataBlocks[index];
                if (endAddresss + 1 >= next.StartAddress)
                {
        ...
        class DummyDataBlock<TData, TRawData> : ModbusDataBlock<TData, TRawData> 
        {
            ...
            public override ushort Count => (ushort)(EndAddress - StartAddress + 1);
