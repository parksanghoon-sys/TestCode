...
namespace BytePacketSupport.BytePacketSupport.Services.CRC
{
    ...
    {
        private readonly CRC8Type _type;
        private readonly byte[] _crc8ClassicTable = // CCITT?
{
        ...
        private readonly byte[] _crc8MaximTable =
        {
        ...
        {
            byte ret = 0x00;
            if (_type == CRC8Type.Classic)
                ret = ComputeClassicCRC8(data);
            else if (_type == CRC8Type.Maxim)
                ret = (ComputeMaximCRC8(data));
        ...
        private byte ComputeMaximCRC8(ReadOnlySpan<byte> data)
        {
            if (data.Length == 0)
                return 0;
