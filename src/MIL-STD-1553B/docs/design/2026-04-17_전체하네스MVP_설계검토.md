...
- 타임아웃 감지, 단일 재시도, 수동 Bus A/B 전환을 포함한 건강 상태 추적
- JSON 기반 시나리오 및 설정 로딩
- BM time-tag 기록, JSONL 영구 저장, Host 조회용 이벤트 스트림, Host report export 초안
- Command Word / Status Word / Data Word / 일부 Mode Code 처리를 위한 구조 정의
- `src/native`, `src/dotnet`, `tests/native`, `tests/dotnet`를 기준으로 한 구현 뼈대 정의

## 4. 제외 범위

- 특정 벤더 SDK 세부 API와 초기화 절차 확정
- 자동 failover 임계치와 복구 정책의 세밀한 튜닝
- WPF / WinUI / CLI 중 최종 Host 프레임워크 확정
...
5. Bus A/B 상태는 명시적으로 표현되어야 하며, MVP에서는 수동 전환과 건강 상태 스냅샷을 우선 지원해야 한다.
...
- Mode Code 영향
  MVP에서는 전체 Mode Code 지원보다, no-data / with-data 구분과 미지원 응답 경로를 먼저 고정한다. 대표 Mode Code는 시뮬레이터 시나리오에서 상태 점검용 최소 집합만 사용하고, 초기 집합은 `Transmit Status Word`, `Transmit BIT Word`로 시작한다.
- Bus A/B 전환 영향
  MVP에서는 수동 전환을 우선 제공하고, 자동 failover는 정책 인터페이스와 상태 모델만 먼저 확보한다. Health Snapshot은 active bus, standby bus, 최근 timeout, retry 누적값을 반드시 포함해야 한다.
...
| Presentation / Host | CLI/UI, 설정 편집, 세션 제어, 조회 모델, 리포트 | `src/dotnet/Host`, `src/dotnet/Interop` |
...
| `HealthSnapshot` | active bus, timeout 수, retry 수, degraded 여부를 담는 상태 객체 |
...
| `IHealthPolicy` | timeout, retry, degraded, bus switch 가능 여부 판단 |
...
| `SimulatorBusAdapter` | RT 송수신 버퍼와 대표 Mode Code를 재현하는 MVP용 Infrastructure 어댑터 |
...
- 초기 영구 저장 포맷은 line-by-line append가 쉬운 JSONL을 사용하고, replay 또는 CSV/Markdown export는 Host 계층에서 후속 확장한다.
- 초기 report export 구현은 `SessionExecutionReport`를 입력으로 받아 summary line 1건과 telemetry event line N건을 출력하는 JSONL exporter로 시작한다.

### 10.5 C# ↔ C++ 경계 원칙

- C#은 raw word 버퍼와 벤더 핸들을 직접 다루지 않는다.
- C ABI는 저수준 `SendWord` 중심이 아니라 세션/유스케이스 중심으로 노출한다.
- 우선 필요한 경계 함수는 `OpenSession`, `StartScenario`, `StopSession`, `SwitchBus`, `GetHealthSnapshot`, `PollEvents`, `InjectFault` 수준으로 제한한다.
- `PollEvents`는 Host DTO에 직접 매핑 가능한 텔레메트리 이벤트 목록을 반환하고, raw binary frame dump 대신 JSONL 저장 포맷과 동일한 의미 단위를 공유한다.
...
- `PollEvents`는 pending telemetry 이벤트를 JSON 배열로 반환하고, 성공 반환 후 세션의 pending 이벤트를 비운다.
...
- managed packaging 루트는 `src/dotnet/MilStd1553.Interop`로 고정하고, `dotnet build` 또는 `dotnet publish` 1회로 `MilStd1553.Interop.dll`, `MilStd1553.Host.dll`, `MilStd1553.Native.dll`가 같은 출력 루트 아래에 배치되도록 한다.
...
- BC 주기 실행, RT 기본 응답, BM 기록, 수동 버스 전환, Health Snapshot 갱신, RT ↔ RT, 대표 Mode Code 2개만 먼저 통과시킨다.
- Host는 세션 시작/중지/조회/버스 전환의 얇은 흐름과 JSONL report export 최소 경로만 연결한다.

### 12.3 Refactor

- 벤더 어댑터와 시뮬레이터 어댑터가 공유하는 공통 정책을 Application/Infrastructure 경계로 재정리한다.
- 오류 코드 변환과 DTO 매핑을 전용 mapper로 분리한다.
- BM 기록 저장 경로와 Host 조회 모델을 분리해 replay/export 확장을 준비한다.
...
- Bus A/B 전환
  수동 전환 성공, 전환 이후 active bus 반영, BM 로그의 bus 라벨 기록을 검증한다.
- 오류 주입
...
| 자동 failover 요구 조기 확장 | 초기 구현 범위가 과도하게 커질 수 있다. | MVP는 수동 전환과 health 모델만 먼저 구현하고, 자동 정책은 backlog로 분리한다. |
...
- Bus A/B, timeout, retry, BM 기록, Mode Code 확장 포인트가 설계에 반영되어 있다.
